...
-- this should be used as the base for any microcontrollers attempting to connect to the network
...
local function sendmessage(command, arguments)
	...
	local functions = {
		...
		DepositItem = function(item, quantity)
			...
			storageserver:Send("Moveitem", "Deposit", item, quantity,bins.Hatch)
		...
		WithdrawItem = function(item, quantity)
			...
			storageserver:Send("Moveitem", "Withdraw", item, quantity,bins.Hatch)
			local val = { Microcontroller:Receive() }
...
Moveitem = function(requestingmicro, resources, action, itemtype, quantity,clienthatch)
		...
		local function Deposit(itemtype, quantity,clienthatch)
			local resource = resources[itemtype].Totalresource
